...
function readJson(filePath) {
  try {
    return { value: JSON.parse(readText(filePath)), error: null };
  } catch (error) {
...
function auditRenovate(repoRoot) {
...
function auditCodingTooling(repoRoot) {
...
function auditAgentGuidance(repoRoot) {
...
function auditConventions(repoRoot) {
...
export function auditBoringFoundationV1(repoRoot) {
  const components = {
...
function applyEnvironment(repoRoot, state, changed) {
  ...
  // A custom existing script is repository-owned. Never overwrite it as part of
  // the broad migration merely to make it match the current scaffold bytes.
  if (existsSync(scriptPath) && readText(scriptPath) !== ENVIRONMENT_SCRIPT) return;
  ...
  for (const file of applied.changed) {
    if (file === ".repository-environment.toml" && beforeConfig) continue;
    if (file === "scripts/codex-environment.sh" && beforeScript) continue;
    changed.push(file);
  }
}

function applyRenovate(repoRoot, state, changed) {
  if (state.status === "valid" || state.status === "conflict") return;
  ...
  if (!Array.isArray(nextExtends) || nextExtends.some((entry) => typeof entry !== "string")) return;
  if (nextExtends.includes(RENOVATE_PRESET)) return;
...
export function applyBoringFoundationV1(repoRoot) {
