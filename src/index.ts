...
function readJson(filePath: string): JsonObject {
  return JSON.parse(readText(filePath)) as JsonObject;
}
...
function ensureConfig(repoRoot: string): ScaffoldConfig {
  const configPath = path.join(repoRoot, ".platform-upgrader.json");
  if (!existsSync(configPath)) {
    throw new Error(`Missing required config: ${configPath}`);
  }

  const config = readJson(configPath) as ScaffoldConfig;
  if (
    !Array.isArray(config.migrationsAccepted) ||
    !config.migrationsAccepted.includes("scaffold-v2")
  ) {
...
function upsertAppManifest(appManifestPath: string): boolean {
...
function normalizePackageJson(packageJsonPath: string): boolean {
...
function ensureMonorepoBaseline(repoRoot: string): boolean {
...
function ensureUpdateGuide(repoRoot: string): boolean {
...
function renameIfExists(repoRoot: string, fromRelative: string, toRelative: string): boolean {
...
function removeIfExists(repoRoot: string, relativePath: string): boolean {
...
function workflowDisplayName(workflowName: string): string {
  if (workflowName === "main.yml") {
    return "Main";
  }
...
function tierWorkflowDetails(workflowName: string): {
  workflowLabel: string;
  checksCommand: string;
} {
  if (workflowName === "beta-tier.yml") {
    return { workflowLabel: "Beta Tier", checksCommand: "bun run checks:beta" };
  }
  if (workflowName === "main-tier.yml") {
    return { workflowLabel: "Main Tier", checksCommand: "bun run checks:main" };
  }
  return { workflowLabel: "Nightly Tier", checksCommand: "bun run checks:nightly" };
}

function normalizeWorkflow(repoRoot: string, workflowName: string): boolean {
...
export function auditRepo(repoRoot: string): {
...
export function applyScaffoldV2(repoRoot: string): {
  repoName: string;
  changed: string[];
} {
