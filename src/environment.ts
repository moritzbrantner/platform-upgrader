...
function bunPin(repoRoot: string): BunPin {
...
function detectedEnvironment(repoRoot: string): { setup: string[]; cache: string[] } {
  const setup: string[] = [];
  const cache: string[] = [];
...
function renderEnvironmentConfig(repoRoot: string): string {
...
function exactBunPinIssue(repoRoot: string): string | null {
  const { packageManagerVersion, versionFileVersion } = bunPin(repoRoot);
...
function exactNodePinIssue(repoRoot: string): string | null {
  ...
  return `.node-version must pin Node exactly, got ${version}`;
}

function exactRustPinIssue(repoRoot: string): string | null {
  ...
  if (!version) {
    return "rust-toolchain.toml has no toolchain channel";
  }
  if (/^\d+\.\d+\.\d+$/.test(version)) {
    return null;
  }
  return `rust-toolchain.toml must pin Rust exactly, got ${version}`;
}

export function auditEnvironmentV1(repoRoot: string): { issues: string[]; ok: boolean } {
  const issues: string[] = [];
...
export function applyEnvironmentV1(repoRoot: string) {
